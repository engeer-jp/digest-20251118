...
以下のスクリプトを実行してフォルダ内のメディア情報をJSON形式で取得します:

```powershell
a:\SlideShow\_shared\skills\.claude\skills\create-digest\scripts\scan-media.ps1 -FolderPath "<フォルダパス>"
```

対象ファイル形式: `*.jpg, *.jpeg, *.png, *.heic, *.mp4, *.mov`
